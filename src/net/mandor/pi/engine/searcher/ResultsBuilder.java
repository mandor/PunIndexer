...
final class ResultsBuilder {
	...
	private static final Logger L = Logger.getLogger(ResultsBuilder.class);
	 ...
	 */
	public ResultsBuilder(final IndexSearcher is, final Type t) {
		searcher = is;
		type = t;
		results = new HashSet<Match>();
	}
	
	/** @return  Collection of {@link Match}es created by this builder. */
	public List<Match> getResults() { return new ArrayList<Match>(results); }

	/** @param td Results to add to the collection of {@link Match}es. */
	public void addTopDocs(final TopDocs td) {
		if (td.totalHits == 0) { return; }
	...
	/** @param f Filter to apply to the existing results. */
	public void applyFilter(final Filter f) { f.filter(results); }
